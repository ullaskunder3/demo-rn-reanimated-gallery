        ...
        <View style={{ flex: 1, justifyContent: "flex-end", backgroundColor: "black" }}>
            ...
            <Animated.FlatList
                ...
                renderItem={({ item, index }) => {
                    return <CarosalItem imageUrl={item} index={index} scorllX={scrollX} />;
                }}
                horizontal
...
function CarosalItem({ imageUrl, index, scorllX }: { imageUrl: string; index: number, scorllX: SharedValue<number> }) {
    const styleZ = useAnimatedStyle(() => {
        return {
            transform: [{
                translateY: interpolate(scorllX.value, [
            ...
            borderWidth: 3,
            borderColor: interpolateColor(scorllX.value,
                [index - 1, index, index + 1],
